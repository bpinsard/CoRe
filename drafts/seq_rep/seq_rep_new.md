...
This is notably reflected by an increase of activity in the caudate nuclei associated with lower execution variability [@albouy_neural_2012], a progressive shift from associative and premotor to sensorimotor basal ganglia distributed network concurring to the automation of the skill [@lehericy_distinct_2005] and further activation of the putamen during and after consolidation of a motor memory trace [@debas_brain_2010;@albouy_hippocampus_2013;@debas_off-line_2014;@fogel_reactivation_2017;@vahdat_network-wide_2017].
...
These studies suggest that acquisition of motor sequences induces local tuning of activity in cortical networks, that is strengthened when practice extends over multiple days and the structured neuronal activity evokes macroscopic BOLD pattern which can be differentiated between sequences using multivariate approaches.
Interestingly the regions carrying such patterns only partly overlap with GLM-based measures that, by directly assess coarser differences between novel and trained sequences evoked activity level, cannot assess if these purport sequential information.

However, as a single-session training followed by an offline period including sleep triggers a reorganization of activity, not only in cortical areas but also in a subcortical MSL network, we here intend to assess in which of these regions does genuine sequence-specific plasticity occur during this earlier stage of MSL.
