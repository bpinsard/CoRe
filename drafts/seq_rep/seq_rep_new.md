...
Therefore, it is thought that sleep favors reprocessing of the motor memory trace, thus promoting its consolidation for long-term skill proficiency [@fischer_sleep_2002;see @king_sleeping_2017;@doyon_current_2018 for a recent in-depth reviews].
...
![Average and standard deviation of correct sequence durations across the MVPA task blocks.](../../results/behavior/mvpa_task_groupInt_seq_duration.pdf){#fig:mvpa_task_groupInt_seq_duration width=14cm}
...
**However, the ongoing debate about the level of M1 control on motor actions, either as low-level movement or higher-level complex kinematics representations [@omrani_perspectives_2017], calls for caution in interpreting these results.
In the context of MVPA analysis of fMRI data during MSL, previous reports of such a representation in primary regions [@wiestler_skill_2013] has recently been revised, being likely explained by the stronger activity evoked by the first finger press in the sequence [@yokoi_does_2017].
In our study, the new sequences started with different fingers and this first-finger effect could explain their discrimination, but the consolidated sequences for which first finger press was similar were nevertheless discriminable.
Although conjectural, this pattern separability could have been driven by the representation of another low-level feature, such as the finger initiating a second chunk in the sequence, but could also reflect afferent signal from secondary motor regions due to the post-synaptic origin of BOLD effect [@logothetis_neurophysiological_2001].
While being specifically located around the hand knob, the spatial extent of the M1/S1 representation was smaller compared to that found by @wiestler_skill_2013.
This is likely explained by differences in our design, notably the uninterrupted repetition of the motor sequences during practice that singularize the effect of the initiating finger to the beginning of the block.
...
**In sum, the network reflecting sequence information shows some overlap with the main effect of activation-based studies [@hardwick_quantitative_2013;@dayan_neuroplasticity_2011], but also reveals discrepancies.
These could be attributable to the use of an information-based inference rather than one based on activation level, and the recruitment of additional regions for the processing of sequential information in stimuli and its maintenance in working memory that the task requires.**
...
**Importantly, no significant representational changes were found in the contralateral primary somatosensory cortex after consolidation, which representations are strongly shaped by ecological finger co-activations [@ejaz_hand_2015] and seems to be resistant to extensive training of a sequence built on a new co-activation structure [@beukema_binding_2018].
While the role of the motor cortex in sequence learning is undeniable, the distributed redundant representation of a trained skill in subcortical regions could allow its expression, yet suboptimal, after a cortical insult or rapidly after M1 inactivation [@kawai_motor_2015;@bollu_cortical_2018].**

Interestingly, significant differences at the subcortical level were found in bilateral putamen and more specifically in their sensorimotor regions, a result consistent with findings from activation studies that reported increased functional activity after consolidation in this structure [@debas_brain_2010;@albouy_hippocampus_2013;@debas_off-line_2014;@fogel_reactivation_2017;@vahdat_network-wide_2017].
...
Indeed, our results uncover that this shift in activity purports a genuine reorganization of circuits processing sequence-specific information, similar to what was reported at the neuronal level in animals [@costa_differential_2004;@yin_dynamic_2009;@miyachi_differential_2002].
...
**Moreover, the training sessions of consolidated sequences were each comprised of a single sequence performed in blocks longer than in the present task designed for multivariate investigation.
The latter further inducing demands for instruction processing, retention in working memory, switching and inhibition of other sequences could have triggered some novel learning for the consolidated sequences, which however do not reflect in performance evolution.
This alternate production of different sequences required shifting between overlapping sets of motor commands, and could have further implicated the dorsal striatum in collaboration with the prefrontal cortex [@monchi_functional_2006] and influence the pattern of results found here.
**
...
**Importantly, these results reveals for the first time in humans that such changes are underlain by the local implementation of distributed neural coding of sequential information.**
